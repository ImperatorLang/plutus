...
-}
import UntypedPlutusCore.Evaluation.Machine.Cek.Internal hiding (runCekDeBruijn)
...
computeCek !unbudgetedSteps !ctx !env (Force _ body) = do
    ...
    pure $ Computing unbudgetedSteps' (FrameForce ctx) env body
-- s ; ρ ▻ [L M]  ↦  s , [_ (M,ρ)]  ; ρ ▻ L
computeCek !unbudgetedSteps !ctx !env (Apply _ fun arg) = do
    !unbudgetedSteps' <- stepAndMaybeSpend BApply unbudgetedSteps
    pure $ Computing unbudgetedSteps' (FrameApplyArg env arg ctx) env fun
...
returnCek !unbudgetedSteps (FrameApplyArg argVarEnv arg ctx) fun =
    pure $ Computing unbudgetedSteps (FrameApplyFun fun ctx) argVarEnv arg
